...
export default function ProcurementDocumentPage({ config }) {
  ...
  return (
    <Box sx={{ flexGrow: 1, padding: { xs: 2, md: 3 }, minHeight: "100vh", backgroundColor: uiColors.bg }}>
      ...
      <Dialog open={dialogMode === "add" || dialogMode === "edit"} onClose={() => setDialogMode(null)} maxWidth="lg" fullWidth>
        <DialogTitle>{dialogMode === "add" ? `Add ${config.title}` : `Update ${config.title}`}</DialogTitle>
        <DialogContent dividers>
          ...
          <Box sx={{ mt: 3 }}>
            <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 1.5 }}>
              <Typography variant="h6" sx={{ fontWeight: 800, color: uiColors.navy }}>Line Items</Typography>
              <Button startIcon={<AddIcon />} onClick={() => setForm((current) => ({ ...current, line_items: [...current.line_items, makeLineItem()] }))} sx={{ textTransform: "none" }}>Add Line</Button>
            </Stack>
            <Stack spacing={1.25}>
              {form.line_items.map((item, index) => (
                <Paper key={`line-item-${index}`} variant="outlined" sx={{ p: 1.5 }}>
                  <Grid container spacing={1}>
                    ...
                    <Grid item xs={12} md={1.5}><TextField label="Amount" fullWidth value={item.amount} onChange={handleLineItemChange(index, "amount")} /></Grid>
                    <Grid item xs={12} md={0.5}>
                      <IconButton onClick={() => setForm((current) => ({ ...current, line_items: current.line_items.length === 1 ? current.line_items : current.line_items.filter((_, itemIndex) => itemIndex !== index) }))}>
                        <RemoveIcon />
                      </IconButton>
                    </Grid>
                  </Grid>
