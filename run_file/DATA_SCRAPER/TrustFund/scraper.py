...
QUERY_FILE = MODULE_DIR / "trust_fund_query.sql"
...
def build_query(start_date: str | None, end_date: str | None) -> str:
    ...
    if filters:
        query += "\nAND " + "\nAND ".join(filters)
    query += "\nGROUP BY p.PAYMENT_ID, p.PAYMENTDATE, p.PAIDBY, p.RECEIPTNO, p.AFTYPE, COALESCE(NULLIF(p.COLLECTOR, ''), p.USERID)"
    query += "\nORDER BY p.PAYMENTDATE DESC, p.RECEIPTNO DESC"
    return query


def main() -> int:
    parser = argparse.ArgumentParser(description="Scrape Trust Fund data from Firebird into MySQL.")
    ...
    try:
        ...
        with OUTPUT_FILE.open("w", encoding="utf-8") as handle:
            ...
            for row in rows:
                ...
                handle.write(f"INSERT INTO trust_fund_payment ({column_sql}) VALUES ({values_sql});\n")
